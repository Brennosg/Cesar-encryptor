...
def caesar_shift(message, shift):
    """
    Aplica a cifra de César a uma mensagem.
    ...
    """
    result = ""  # Espaço para armazenar a mensagem criptografada.
    ...
    for char in message.upper():
        if char.isalpha():  # Verifica se o caractere é uma letra.
            ...
            new_char = chr(new_char_code)  # Converte o novo código ASCII de volta para caractere.
...
def get_user_input():
    """
    Obtém a entrada do usuário para a mensagem e a chave de deslocamento.

    Returns:
    tuple: Contendo a mensagem do usuário e a chave de deslocamento.
    """
    user_message = input("Digite a mensagem para criptografar: ")  # Solicita a mensagem do usuário.
    
    # Loop para garantir que o usuário insira um valor inteiro válido para a chave de deslocamento.
    while True:
        try:
            user_shift_key = int(input("Chave de deslocamento (número inteiro): "))  # Solicita a chave de deslocamento.
            break  # Sai do loop se a entrada for um inteiro válido.
        except ValueError:
            print("Por favor, insira um valor inteiro válido para a chave de deslocamento.")  # Mensagem de erro para entrada inválida.
